...
const ProfilePage = ({ user, onBack, onUpdateUsername, onCheckUsername, onDisconnect, onConvertCoins, onJoinTournament, onClaimDaily, onPlay, rank, status, onGoToGameroom, isProcessing }) => {
    ...
    const handleCheck = async () => {
        ...
        setCheckStatus({ available: false, message: "Checking..." });
        const result = await onCheckUsername(newUsername.trim());
            ...
            <nav className="sticky top-0 z-50 w-full border-b-4 border-secondary bg-white/90 backdrop-blur-md">
                <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
                    ...
                    <div className="flex items-center gap-4">
                        ...
                        <button
                            onClick={onGoToGameroom}
                            className="bg-primary text-secondary border-2 border-secondary px-4 py-2 text-xs font-black uppercase tracking-wider pixel-shadow hover:scale-[1.02] active:translate-y-1 transition-all"
                        >
                            Gameroom
                        </button>
                        <button
                            ...
                            className={`bg-red-500 text-white border-2 border-secondary px-4 py-2 text-xs font-black uppercase tracking-wider pixel-shadow transition-all ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'hover:bg-red-600 active:translate-y-1'}`}
                        >
                            {isProcessing ? 'Loading' : 'Disconnect'}
                        </button>
                    </div>
                </div>
            </nav>
                ...
                <div className="bg-white border-4 border-secondary p-6 md:p-10 mb-8 pixel-shadow relative overflow-hidden">
                    ...
                    <div className="relative z-10 grid grid-cols-1 md:grid-cols-3 gap-8 items-center">
                                    ...
                                    <div className="flex flex-col mt-2">
                                        <div className="flex gap-2">
                                            ...
                                            {checkStatus?.available ? (
                                                <button onClick={handleUpdate} disabled={isProcessing} className={`bg-primary border-2 border-secondary px-3 py-1 font-bold ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}>SET</button>
                                            ) : (
                                                <button onClick={handleCheck} disabled={checkStatus?.message === "Checking..." || isProcessing} className={`bg-primary border-2 border-secondary px-3 py-1 font-bold ${isProcessing ? 'opacity-50 cursor-not-allowed' : ''}`}>CHECK</button>
                                            )}
                                            <button onClick={() => { setIsEditing(false); setCheckStatus(null); setNewUsername(user.username); }} className="bg-gray-200 border-2 border-secondary px-3 py-1 font-bold">X</button>
                                        </div>
                                        {checkStatus && (
                                            <p className={`text-[10px] font-bold uppercase mt-1 ${checkStatus.message === "Checking..." ? "text-gray-500" : checkStatus.available ? "text-green-500" : "text-red-500"}`}>
                                                {checkStatus.message}
                                            </p>
                                        )}
                                    </div>
                        ...
                        <div className="col-span-2 grid grid-cols-1 sm:grid-cols-3 gap-4">
                            <div className="bg-primary text-secondary p-6 border-4 border-secondary pixel-shadow-sm text-center flex flex-col justify-between">
                                <p className="text-[10px] font-black uppercase mb-1 opacity-70">$DASH Balance</p>
                                <p className="text-xl font-black font-mono my-auto">{user.dashBalance?.toLocaleString() || 0}</p>
                                <p className="text-[8px] font-bold uppercase mt-2 opacity-60">*Non-Transferable</p>
                            </div>
                            <div className="bg-accent-gray p-6 border-4 border-secondary pixel-shadow-sm text-center relative overflow-hidden flex flex-col justify-between">
                                <p className="text-[10px] font-bold text-secondary uppercase mb-1">Est. Winnings</p>
                                <p className="text-xl font-black font-mono text-primary my-auto">0 $DASH</p>
                                <p className="text-[8px] font-bold uppercase mt-2 text-gray-400">Current Rank: --</p>
                            </div>
                            <div className="bg-white p-6 border-4 border-secondary pixel-shadow-sm text-center relative overflow-hidden flex flex-col justify-between">
                                <p className="text-[10px] font-bold text-secondary uppercase mb-1">Start Running</p>
                                <div className="my-auto py-1">
                                    <button 
                                        onClick={() => onPlay(false)}
                                        disabled={isProcessing}
                                        className={`w-full bg-primary text-secondary py-2 px-3 text-xs font-black uppercase border-2 border-secondary pixel-shadow-sm transition-all flex items-center justify-center gap-1 ${isProcessing ? 'opacity-50 cursor-not-allowed' : 'hover:scale-[1.02] active:translate-y-0.5'}`}
                                    >
                                        <span className="material-symbols-outlined text-sm">play_arrow</span>
                                        {isProcessing ? 'Loading...' : 'Play'}
                                    </button>
                                </div>
                                <p className="text-[8px] font-bold uppercase text-gray-400">Ranked Mode: Soon</p>
                                
                                {/* Status Overlay for Play Card */}
                                {status && (
                                    <div className="absolute inset-0 bg-secondary/90 backdrop-blur-sm flex items-center justify-center p-2 text-center z-50">
                                        <div className="inline-block animate-spin size-5 border-2 border-primary border-t-transparent rounded-full"></div>
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white border-4 border-secondary p-6 pixel-shadow relative overflow-hidden group">
                            <div className="relative z-10">
                                ...
                                >
                                    ...
                                    {isProcessing ? 'Loading...' : (canClaimDaily ? 'Sign-In (Gas Only)' : 'Check Back Tomorrow')}
                                </button>
                            ...
                            <div className="relative z-10">
                                ...
                                <div className="space-y-4">
                                    ...
                                    <button 
                                        ...
                                        disabled={!user.gameCoins || user.gameCoins < 1000 || isProcessing}
                                        className={`w-full py-4 text-sm font-black uppercase tracking-widest border-4 pixel-shadow transition-all flex items-center justify-center gap-2
                                            ${(user.gameCoins >= 1000 && !isProcessing) 
                                                ? 'bg-primary text-secondary border-white hover:bg-white hover:scale-[1.02] active:translate-y-1' 
                                                : 'bg-gray-800 text-gray-500 border-gray-700 cursor-not-allowed opacity-50'}`}
                                    >
                                        <span className="material-symbols-outlined">account_balance_wallet</span>
                                        {isProcessing ? 'Processing...' : (user.gameCoins >= 1000 ? `Exchange for ${Math.floor(user.gameCoins / 1000) * 100} $DASH` : 'Need 1,000 Coins')}
                                    </button>
